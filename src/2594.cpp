...
class Solution {
  public:
    long long repairCars(vector<int> &ranks, int cars) {
        int sizeRanks = ranks.size();
        long long minimumTime = 0, maximumTime = 100LL * 1000000LL * 1000000LL;

        while (minimumTime < maximumTime) {
            long long midTime = (minimumTime + maximumTime) / 2;
            int tmpCars = 0;

            for (int i = 0; i < sizeRanks && tmpCars < cars; i++) {
                tmpCars += (long long)sqrt((double)midTime / ranks[i]); // t = r * n * n => n = sqrt(t / r)
            }
...
int main() {
    TestSuite<long long, vector<int>, int> testSuite("Minimum time to repair card", "ranks", "cars");
    ...
    Main main;
    main.runTests(&Solution::repairCars, testSuite);
}
